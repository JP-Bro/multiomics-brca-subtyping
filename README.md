...
```
multiomics-brca-subtyping/
├── data/
│   ├── raw/                  # TCGA-BRCA source files (download separately, see Setup)
│   ├── processed/            # merged data, test set, metadata (generated by notebooks)
│   └── sample_patients/      # example CSVs for testing the live demo
├── notebooks/
│   ├── 01_data_exploration.ipynb       # loading raw files, finding PAM50 label, checking overlap
│   ├── 02_preprocessing.ipynb          # merging, feature filtering, train/val/test split
│   ├── 03_xgboost_baseline.ipynb       # XGBoost model, marker gene fix, evaluation
│   ├── 04_autoencoder_fusion.ipynb     # PyTorch autoencoder fusion model, training, ablation
│   └── 05_evaluation_comparison.ipynb  # SHAP analysis, confusion matrices, final comparison
├── src/
│   ├── models.py             # MultiOmicsAutoencoder architecture definition
│   ├── preprocessing.py      # reusable data preprocessing functions
│   └── evaluation.py         # reusable evaluation/metrics functions
├── results/
│   ├── models/                # saved trained models (xgboost_model.pkl, autoencoder weights)
│   └── figures/                # SHAP plots, confusion matrices, comparison table
├── app/
